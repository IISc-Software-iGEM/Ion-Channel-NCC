...
def filemaker(protein, pqr_file, pot_dx_file):
    ...
    ATOM   N  Residue A Resi_num    x y z cx cy cz q r potential
    ...
    with open("/Volumes/Anirudh/IISc/IGEM/Ion-Channel-NCC/codes/potential_analyse/potential.txt", "w") as p:
        ...
        for line in pqr_data:
            ...
            p.write("{:<10} {:<6} {:<6} {:<6} {:<6} {:<10} {:<10} {:<10} {:<10} {:<10}\n".format(
                '    '.join(line[:-5]), str(x), str(y), str(z), str(cx), str(cy), str(cz), str(q), str(r), str(potential)))
            i+=1
            if i>=10:
                break
    return
